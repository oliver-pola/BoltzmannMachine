...
COLLECT-STATISTICS():
  FOR 1 to number of co-occurance_epochs:
    FOR 1 to number of unclamped units:
      randomly select an unclamped unit i
      Calculate its energy difference: (*$ \Delta E_i = \sum_j W_{ij} x_j $*)
      #Stochastically decide the resulting state
...
\begin{enumerate}
    ...
    \item \textbf{The free-running phase:}\newline
        ...
        is analyzed by the network and found correlations between for example two connected units often beeing active at the
        same time are strengthened. The process of finding a more optimal low energy state mainly occures in this phase.\newline
    ...
    \item \textbf{Updating the weights:}\newline
        The updating of the weights during the learning phase is done by comparing statistics of so called co-occuring units in the positive
        and in the negative phase. A co-occurance is the case that two units $i,j$ connected by a weight $w_{ij}$ are both observed in the
        state $1$. In the pseudo code of the algorithm these co-occurances are summed up in the vector $S_{ij}$, where the field $ij$ is increased
        by one every time the units $i,j$ have both been observed in the state $1$.
        Since we are dealing with a stochastic process here it is neccessary to collect these statistics over multiple epochs and to average
        the results afterwards. This can be seen at the definitions of the $P^+_{ij}$ and $P^-_{ij}$ variables.\newline
        $P^+_{ij}$ holds the co-occurance statistics for the positive phase and $P^-_{ij}$ holds the statistics for the negative phase.
        It is important to recognize that the collection of the statistics in the algorithm only starts after a good amount of 
        update steps have already been done for each phase. This is needed because we can only make meaningful asumptions about 
        co-occurances after each phase had a little time to already move towards converging on a low energy state.
        The 'collect-statistics' function then repeats a number of update steps during which all co-occurances are summed up.\newline
        The actual changing of the weights happens after eacht time the a pair of positive and negative phases has finished.
        Here the values of $P^+_{ij}$ and $P^-_{ij}$ are compared for every connected pair of units and their weight
        is increased by a set learning factor $k$ if there where more co-occurances observed in the positive phase over the negative phase
        or it is decreased if less co-occurances where observed in the positive phase. This process reinforces the learned co-occurance 
        ...
        two units often showed a co-occurance that means that this information should be learned and it will therefore
        be reinforced by changing their weight correspondingly. If however in the free-running phase two units
        are co-occuring more often than in the positive phase this does not represent the learned data properly and
        the probability of this co-occurance has to be decreased by a corresponding change of weights.\newline
        This whole process of collecting the co-occurance statistics and then updating the weights is done for
...
\todo{whatever is seen in the T scan varies a lot}
